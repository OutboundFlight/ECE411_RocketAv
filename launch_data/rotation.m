...
data = csvread('./08122016_launch2.csv');

gyro_scale = 2000;
half_range = 2^15;
gyro_convert = gyro_scale/half_range;

%Convert to seconds and normalize to start at 0 for the sake of ease of use
data(:,1) = 1E-3*data(:,1);
steps = length(data);
data(:,1) = data(:,1) - data(1,1);
tstep = mean(diff(data(:,1)));

%Convert x, y and z rate gyro measurements to radians
xrotrate = deg2rad*reshape(data(:,5), 1, steps)*gyro_convert;
yrotrate = deg2rad*reshape(data(:,6), 1, steps)*gyro_convert;
zrotrate = deg2rad*reshape(data(:,7), 1, steps)*gyro_convert;
...
for i = 1:skip:steps
    ...
    hold on
    title(sprintf('Time: %g sec\t Accel: %g sec', data(i, 1), norm([data(i,2), data(i, 3), data(i,4)])))
